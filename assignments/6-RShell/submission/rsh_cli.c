...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        // TODO fgets input
        ssize_t status;
        if ((status = (fgets(cmd_buff, ARG_MAX, stdin) == NULL))) {
            if (status < 0) {
                perror("fgets");
            }
            break;
        ...
        while ((io_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0) {
            ...
            if (rsp_buff[io_size - 1] == RDSH_EOF_CHAR || (io_size == 1 && rsp_buff[0] == RDSH_EOF_CHAR)) {
                is_last_chunk = 1;
            } else {
                is_last_chunk = 0;
            }
...
int start_client(char *server_ip, int port){
    ...
    ret = connect (cli_socket, (struct sockaddr *) &addr,
                   sizeof(addr));
    if (ret == -1) {
    ...
    if (ret < 0) {
        fprintf(stderr, "Problems with cmd loop\n");
        close(cli_socket);
        exit(EXIT_FAILURE);
    }
